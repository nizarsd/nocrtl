...
// 	parameter routerid=-1;
	...
	(* dont_touch = "true" *)  output [`HDR_SZ + `PL_SZ + `ADDR_SZ-1:0] parallel_out;

	// state 0 = idle, 1 = receiving, 2 = delivering item

	wire [1:0] gstate; // global state 
	
 	(* dont_touch = "true" *)  reg [1:0] rd_state; // read state
 	
 	(* dont_touch = "true" *)  reg [1:0] wr_state; // write state 
	...
	wire validd, readd, read;
 	...
 	(* dont_touch = "true" *) assign parallel_out = item[`HDR_SZ + `PL_SZ + `ADDR_SZ-1:0];

//	(* dont_touch = "true" *) assign parallel_out = {item[`HDR_SZ + `PL_SZ-1:0],4'b1};
